...
(function($) {
	...
	jQuery(document).ready(function($) {
	 ...
	 if(active_payment_gateway=='stripe'){
		Stripe.setPublishableKey(tiger_data.stripe_publishable);
		...
		jQuery.ajax({					
			...
			success : function(response){
				if(response.code=='success'){							
					jQuery('#coupon-result').html('<img src="'+tiger_data.right_icon+'">');							
					
				}else{
					jQuery('#coupon-result').html('<img src="'+tiger_data.wrong_icon+'">');
				}
...
function show_coupon(){
				jQuery("#coupon-div").show();
                 jQuery("#show_hide_div").html('<label for="text" class="col-md-3 control-label"></label><div class="col-md-9 " ><button type="button" onclick="hide_coupon();"  class="btn btn-default center"> '+tiger_data.Hide_Coupon +'</button></div>');
}
function hide_coupon(){
				 jQuery("#coupon-div").hide();
                 jQuery("#show_hide_div").html('<label for="text" class="col-md-3 control-label"></label><div class="col-md-9 " ><button type="button" onclick="show_coupon();"  class="btn btn-default center"> '+tiger_data.have_Coupon +'</button></div>');
}
