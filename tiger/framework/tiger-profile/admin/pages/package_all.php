<?php
	...
	//include( wp_uou_tigerp_ABSPATH. 'install/install-order-email.php');
	//include( wp_uou_tigerp_ABSPATH. 'install/install-signup-email.php');
...
if(isset($_REQUEST['delete_id']))  {
	$post_id=$_REQUEST['delete_id'];
		...
		$recurring= get_post_meta($post_id, 'uou_tigerp_package_recurring', true);
...
<script>
	...
	function save_package_table(){
						...
						var search_params = {
							"action": 			"uou_tigerp_update_price_table_template",
...
function iv_package_status_change(status_id,curr_status){
					...
					var ajaxurl = '<?php echo admin_url('admin-ajax.php'); ?>';
						var search_params = {
							"action": 	"uou_tigerp_update_package_status",
							"status_id": status_id,	
	...
	<div class="welcome-panel container-fluid">
			...
			<div class="row ">					
				<div class="col-md-12" id="submit-button-holder">					
					<div class="pull-right ">								
						...
						<a class="btn btn-info "  href="<?php echo wp_uou_tigerp_ADMINPATH; ?>admin.php?page=wp-uou_tigerp-package-create"><?php _e( 'Create A New Package', 'tiger' ); ?></a>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-12 table-responsive">
					...
					<h3  class=""><?php _e( 'All Package / Membership Level', 'tiger' ); ?>  <small></small>
						<small >
							<?php
				...
				<div class="panel panel-info">
					...
					<table class="table table-striped col-md-12">
						<thead >
							<tr>
								<th ><?php _e('Package Name','tiger'); ?></th>
								<th ><?php _e('Amount','tiger'); ?></th>
								<th><?php _e('Link','tiger'); ?></th>
								<th><?php _e('Type','tiger'); ?>   </th>
								<th><?php _e('User Role','tiger'); ?></th>
						...
						<tbody>
							...
							if(sizeof($membership_pack)>0){
								...
								{	
									...
									$amount='';
									if(get_post_meta($row->ID, 'uou_tigerp_package_cost', true)!="" AND get_post_meta($row->ID, 'uou_tigerp_package_cost', true)!="0"){
										$amount= get_post_meta($row->ID, 'uou_tigerp_package_cost', true).' '.$currency;
									}else{
										$amount= '0 '.$currency;
									}
									...
									$recurring= get_post_meta($row->ID, 'uou_tigerp_package_recurring', true);	
									if($recurring == 'on'){
										$amount= get_post_meta($row->ID, 'uou_tigerp_package_recurring_cost_initial', true).' '.$currency;
									...
									echo '<td>'.ucfirst(get_post_meta($row->ID, 'uou_tigerp_user_type', true)).'</td>';
									echo '<td>'.get_post_meta($row->ID, 'uou_tigerp_user_type', true).'-'. $row->post_title.'</td>';
									...
									<button class="btn btn-info btn-xs" onclick="return iv_package_status_change('<?php echo $row->ID; ?>','<?php echo $row->post_status; ?>');"><?php echo $pac_msg; ?></button>
										...
										<?php
										echo" </td> ";
										echo '<td>  <a class="btn btn-primary btn-xs" href="?page=wp-uou_tigerp-package-update&id='.$row->ID.'"> Edit</a> <a href="?page=wp-uou_tigerp-package-all&delete_id='.$row->ID.'" class="btn btn-danger btn-xs" onclick="return confirm(\'Are you sure to delete this package?\');">Delete</a></td>';
					...
					<div class="col-md-12">					
						<div class="">								
							...
							<a class="btn btn-info "  href="<?php echo wp_uou_tigerp_ADMINPATH; ?>admin.php?page=wp-uou_tigerp-package-create"><?php _e( 'Create A New Package', 'tiger' ); ?> </a>
								...
								<?php //echo $page_name_reg=get_option('_uou_tigerp_registration' );  ?>
								<!--
								<a  class="btn btn-primary " target="blank"href="<?php  $page_name_reg=get_option('_uou_tigerp_registration' ); 	  echo get_page_link($page_name_reg); ?>">Registration Page</a>
								-->
						</div>
					</div>
				</div>
		...
		<div class="panel panel-info">
			<div class="panel-body">		
					...
					<?php _e( 'User role "Basic" is created on the plugin activation. Paid exprired or unsuccessful payment user will set on the "Basic" user role.', 'tiger' ); ?>
						...
						<ul class=" list-group col-md-6">
								<li class="list-group-item">Short Code : <code>[uou_tigerp_price_table]  </code></li>
								<li class="list-group-item">PHP Code :<code>
											&lt;?php
											echo do_shortcode('[uou_tigerp_price_table ]');
											?&gt;</code>  
										</li>
								<li class="list-group-item">
									<?php _e( 'Package Page :', 'tiger' ); ?>  
									<?php 
											$uou_tigerp_price_table=get_option('_uou_tigerp_price_table');
											  ?>
											<a class="btn btn-info btn-xs " href="<?php echo get_permalink( $uou_tigerp_price_table ); ?>" target="blank"><?php _e( 'View Page', 'tiger' ); ?></a>
					...
					<div class="  bs-callout bs-callout-info">	
						<?php _e( 'Note: You can use other available pricing table. The package link URL will go on "Sign UP " button.', 'tiger' ); ?>
