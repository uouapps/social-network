				...
				<?php
                  ...
                  <div class="filter-flower">
                    <div class="row">
                      <div class="col-sm-7">
                        <ul>
                          <li><a href="<?php echo the_permalink();?>?&profile=network&network=following&following-type=All" <?php echo($connection_type_following=='All' ?'class="active"':'' ); ?> ><?php esc_html_e('Show All','tiger'); ?></a></li>
                          <li><a href="<?php echo the_permalink();?>?&profile=network&network=following&following-type=Professionals" <?php echo($connection_type_following=='Professionals' ?'class="active"':'' ); ?> ><i class="fa fa-user"></i> <?php esc_html_e('Professionals','tiger'); ?></a></li>
                          <li><a href="<?php echo the_permalink();?>?&profile=network&network=following&following-type=Companies" <?php echo($connection_type_following=='Companies' ?'class="active"':'' ); ?> ><i class="fa fa-building-o"></i> <?php esc_html_e('Companies','tiger'); ?></a></li>
                        </ul>
                      </div>
                  ...
                  <div class="main-mem"> 
                    ...
                    <div class="head">
                      <div class="row">
                        <div class="col-sm-8">
                          <button onclick="following_make_unfollow();" ><i class="fa fa-user-times"></i><?php esc_html_e('UnFollow','tiger'); ?></button>
                          <button onclick="following_make_conection();"  ><i class="fa fa-users"></i><?php esc_html_e('Connect','tiger'); ?></button>
                        </div>
                        <div class="col-sm-4">
                           <form action="<?php echo the_permalink().'?&profile=network&network=follower'; ?>" method="post"  >
								<input type="text" id="following_search" name="following_search"  placeholder="<?php esc_html_e('Search','tiger'); ?>" value="<?php echo $search_connection; ?>">
						   ...
						   $user_query = new WP_User_Query( $args );
							...
							if ( ! empty( $user_query->results ) ) {
								...
								if($total_pages_following>1){
									...
									echo'<div class="text-center" id="add_more_following_more_button"><button type="button" onclick="add_more_following_ajax(2);"><i class="fa fa-plus"></i> '.esc_html__(' More','tiger').'</button> </div>';
								}
							} else {?>
								<li>
...
function following_make_unfollow(){
	...
	var search_params={
			"action"  : 	"uou_tigerp_bulk_following_make_unfollow",	
...
function following_make_conection(){
	...
	var search_params={
			"action"  : 	"uou_tigerp_bulk_following_make_connect",	
			"form_data":	jQuery("#following_form_list").serialize(), 
		};
		jQuery.ajax({					
			url : ajaxurl,					 
...
var total_page=<?php echo (isset($total_pages_following)?$total_pages_following:0 ); ?>;	 
	var loader_image = '<img src="<?php echo wp_uou_tigerp_URLPATH. "admin/files/images/loader.gif"; ?>" />';
		...
		var search_params={
			"action"  : 	"uou_tigerp_load_more_following",	
