...
?> 
          <div class="network">
            <h4><?php  esc_html_e('Blog','tiger');?> </h4>
                 
                     <ul class="nav nav-tabs">
					  <li class="active"><a data-toggle="tab" href="#tab_1_3" ><?php  esc_html_e('My All Blog','tiger');?></a></li>	
					  <li ><a data-toggle="tab" href="#tab_1_1"><?php  esc_html_e('Add Blog Post','tiger');?> </a></li>					  
					 ...
					 <table class="table table-striped ">
							...
							<?php
								...
								<tr>
									...
									<td width="20%" >
										<?php											
											$edit_post= $profile_url.'?&profile=blog-edit&post-id='.$row->ID;										
											?>											
										<a href="<?php echo $edit_post; ?>" class="btn btn-xs green-haze" ><?php  _e('Edit','tiger');?></a> 										
										<a href="<?php echo $profile_url.'?&profile=add-news&delete_id='.$row->ID ;?>"  onclick="return confirm('<?php  _e('Are you sure to delete this post?','tiger');?>');"  class="btn btn-xs btn-danger"><?php  _e('Delete','tiger');?>										
										</a></td>
								</tr>
							...
							<div class="center">
								...
								for($i=1;$i<= $total_page;$i++){
										echo '<li class="list-pagi '.($i==$current_page  ? 'active-li': '').'"><a href="'.get_permalink().'?&profile=add-blog&cpage='.$i.'"> '.$i.'</a></li>';		
								...
								<div class=" row form-group ">
									<label for="text" class=" col-md-12 control-label"><?php  _e('Post Status','tiger');?>  </label>
										...
										<select name="post_status" id="post_status"  class="form-control">
											<?php
												$dir_approve_publish =get_option('_blog_approve_publish');
												...
												?>	
													...
													if ( $categories && !is_wp_error( $categories ) ) :
														...
														foreach ( $categories as $term ) { 
															echo '<option  value="'.$term->term_id.'" '.($selected==$term->slug?'selected':'' ).'>--'.$term->name.'</option>';
														} 	
...
function iv_save_post (){
	...
	var loader_image = '<img src="<?php echo wp_uou_tigerp_URLPATH. "admin/files/images/loader.gif"; ?>" />';
				...
				var search_params={
					"action"  : 	"uou_tigerp_save_wp_post",	
				...
				jQuery.ajax({					
					...
					success : function(response){
						if(response.code=='success'){
								var url = "<?php echo get_permalink(); ?>?&profile=add-blog";    						
