								...
								<div class=" row form-group ">
									<label for="text" class=" col-md-12 control-label"><?php  _e('Post Status','wpmembership');?>  </label>
										...
										<select name="post_status" id="post_status"  class="form-control">
											...
											<option value="draft" <?php echo (get_post_status( $post_edit->ID )=='draft'?'selected="selected"':'' ) ; ?> ><?php  _e('Draft','tiger');?></option>
										</select>										
								...
								<div class=" row form-group">
									...
									<div class=" col-md-12 "> 
										<?php
											...
											$currentCategory= $currentCategory = get_the_category($post_edit->ID);
											$selected='';
											if(get_the_category($post_edit->ID)){ 
											$currentCategory = get_the_category($post_edit->ID);  // the value is recieved properly
											$selected = $currentCategory[0]->term_id; // the value is assigned properly
										}
											...
											$categories = get_categories('hide_empty=0'); // Get all terms of a taxonomy								 
													...
													if ( $categories && !is_wp_error( $categories ) ) :
														...
														foreach ( $categories as $term ) { 
															echo '<option  value="'.$term->term_id.'" '.($selected==$term->term_id?'selected':'' ).'>'.$term->name.'</option>';
...
function iv_update_post (){
	...
	var loader_image = '<img src="<?php echo wp_uou_tigerp_URLPATH. "admin/files/images/loader.gif"; ?>" />';
				...
				var search_params={
					"action"  : 	"uou_tigerp_update_wp_post",	
