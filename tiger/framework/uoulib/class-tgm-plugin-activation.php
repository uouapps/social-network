...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( $this->tgmpa->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        ...
                        $table_data[ $i ]['source'] = esc_html__( 'Private Repository', 'tiger' );
                    }
                    else {
                        // The plugin is pre-packaged with the theme.
                        $table_data[ $i ]['source'] = esc_html__( 'Pre-Packaged', 'tiger' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[ $i ]['source'] = esc_html__( 'WordPress Repository', 'tiger' );
                }

                $table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? esc_html__( 'Required', 'tiger' ) : esc_html__( 'Recommended', 'tiger' );

                if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
                    $table_data[ $i ]['status'] = sprintf( '%1$s', esc_html__( 'Not Installed', 'tiger' ) );
                }
                elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[ $i ]['status'] = sprintf( '%1$s', esc_html__( 'Installed But Not Activated', 'tiger' ) );
                }
        ...
        public function column_cb( $item ) {
            ...
            if ( esc_html__( 'Private Repository', 'tiger' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            }
            elseif ( esc_html__( 'Pre-Packaged', 'tiger' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( wp_kses_post( esc_html__( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'tgmpa' ) ), esc_url( self_admin_url() ) );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => esc_html__( 'Plugin', 'tiger' ),
                'source' => esc_html__( 'Source', 'tiger' ),
                'type'   => esc_html__( 'Type', 'tiger' ),
                'status' => esc_html__( 'Status', 'tiger' ),
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => esc_html__( 'Install', 'tiger' ),
                'tgmpa-bulk-activate' => esc_html__( 'Activate', 'tiger' ),
            );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = esc_html__( 'Install package not available.', 'tiger' );
                        $this->strings['downloading_package'] = esc_html__( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tiger' );
                        $this->strings['unpack_package']      = esc_html__( 'Unpacking the package&#8230;', 'tiger' );
                        $this->strings['installing_package']  = esc_html__( 'Installing the plugin&#8230;', 'tiger' );
                        $this->strings['process_failed']      = esc_html__( 'Plugin install failed.', 'tiger' );
                        $this->strings['process_success']     = esc_html__( 'Plugin installed successfully.', 'tiger' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'tiger' );
                        $this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'tiger' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function before( $title = '' ) {
                        ...
                        printf( '<h4>' . wp_kses_post( $this->upgrader->strings['skin_before_update_header'] ). ' <img alt="'.esc_html_e( 'image', 'tiger' ).'" src="' . esc_url( admin_url( 'images/wpspin_light.gif' ) ) . '" class="hidden waiting-' . esc_attr( $this->upgrader->update_current ) . '" style="vertical-align:middle;" /></h4>', esc_html( $this->plugin_names[ $this->i ] ), absint( $this->upgrader->update_current ), absint( $this->upgrader->update_count ) );
