 ...
 * @since 2.8.0
 */
class tiger_Contact_Info extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'tiger_widget_contact_info', 'description' => esc_html__('sb Contact Info widgets','tiger'));
		$control_ops = array('width' => 400, 'height' => 350);
		parent::__construct('tiger_contact_info', esc_html__('tiger Contact Info','tiger'), $widget_ops, $control_ops);
	}
	...
	public function widget( $args, $instance ) {
		...
		$address = apply_filters( 'tiger_mobile_no', empty( $instance['address'] ) ? '' : $instance['address'], $instance, $this->id_base );


		$phone_no = apply_filters( 'tiger_phone_no', empty( $instance['phone_no'] ) ? '' : $instance['phone_no'], $instance, $this->id_base );

		$email = apply_filters( 'tiger_email', empty( $instance['email'] ) ? '' : $instance['email'], $instance, $this->id_base );
		
		echo apply_filters( 'about_before',  $args['before_widget'] );

		if(isset($tiger_option_data['tiger-footer-icon']) && !empty($tiger_option_data['tiger-footer-icon'])){
			$logo = $tiger_option_data['tiger-footer-icon']['url'];	
		}
		...
		<?php } ?>
			...
			if ( ! empty( $bg_image ) ) {
				...
				echo '<ul class="contact-info has-bg-image contain" data-bg-image="'.tiger_IMAGE.''.$bg_image.'">'; 
...
?>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('bg_image')); ?>"><?php esc_html_e('Background Image:','tiger'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('address')); ?>"><?php esc_html_e('Address:','tiger'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('phone_no')); ?>"><?php esc_html_e('Phone No:','tiger'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php esc_html_e('Email :','tiger'); ?></label>
		...
		<p><input id="<?php echo esc_attr($this->get_field_id('filter')); ?>" name="<?php echo esc_attr($this->get_field_name('filter')); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo esc_attr($this->get_field_id('filter')); ?>"><?php esc_html_e('Automatically add paragraphs','tiger'); ?></label></p>
<?php
...
add_action('widgets_init', 'tiger_contact_info');

function tiger_contact_info(){

    register_widget('tiger_Contact_Info');
