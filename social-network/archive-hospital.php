...
wp_enqueue_style('single-hospital-style-2', SB_CSS.'single-hospital.css', array(), $ver = false, $media = 'all');
...
if(get_query_var('hospital-category')!=''){
		$postcats = get_query_var('hospital-category');
		$args['hospital-category']=$postcats;
		$selected=$postcats;

}

if( isset($_POST['hospital-category'])){
	if($_POST['hospital-category']!=''){
		$postcats = $_POST['hospital-category'];
		$args['hospital-category']=$postcats;
		$selected=$postcats;
		$search_show=1;
...
<div class="breadcrumb-content">
		...
		<div class="container">
			<h3>	<?php
				esc_html_e( 'Hospital Directory', 'chilepro' );
			?></h3>
		</div>
				...
				<div class="container">
				  ...
				  <div class="">
					  ...
					  <i class="fa fa-chevron-down arrow"></i>
									<?php
								echo '<select name="hospital-category" class="form-control">';
								echo'	<option selected="'.$selected.'" value="">'.__('Any Category','chilepro').'</option>';


										if( isset($_POST['submit'])){
											$selected = $_POST['hospital-category'];
										}
											//directories
				  ...
				  <!-- <div class="range-slider-content">
							<?php
								...
								?>
							<div class="form-group" >
								<?php esc_html_e( 'Radius', 'chilepro' ); ?>: <span id="rvalue"><?php echo $radius;?></span><?php echo ' '.$dir_search_redius; ?>
							</div>
 ...
 <div class="clearfix top-8" >
    ...
    ?>
	 ...
	 <div id="js-filters-meet-the-team" class="cbp-l-filters-button cbp-l-filters-left" >
			...
			if($postcats==''){	?>
				...
				if ( $categories && !is_wp_error( $categories ) ) :
					foreach ( $categories as $term ) {
						...
						?>
						<div data-filter="" class="cbp-filter-item"><a href="<?php echo get_post_type_archive_link( 'hospital' ).'?&hospital-category='.$term->slug ; ?>">
							<?php echo $term->name; ?>
							</a>
						</div>
					<?php
		...
		if($postcats!=''){ ?>
				<div data-filter="" class="cbp-filter-item"><a href="<?php echo get_post_type_archive_link( 'hospital' ) ; ?>">
					<?php esc_html_e('Show All', 'chilepro' ); ?>
					</a>
				</div>
			...
			<?php
			 ...
			 $custom_cat_obj =  get_term_by('slug',$postcats,'hospital-category');
...
while ( $the_query->have_posts() ) : $the_query->the_post();
			...
			$currentCategoryId='';
			$terms =get_the_terms($id, "hospital-category");
			if($terms!=""){
			...
			array_push( $dirs_data, $dir_data );
				...
				<div class="cbp-item <?php echo $cat_slug; ?> ">
					<a href="<?php echo admin_url('admin-ajax.php'); ?>?action=iv_hospital_ajax_single&id=<?php echo $id; ?>" class="cbp-caption cbp-singlePage" rel="nofollow">
...
<?php 
	if ( !$the_query->have_posts() ){
	esc_html_e( 'Sorry, no hospital matched your criteria.','chilepro' ); 
	}
?>
	 <!--  start pagination * -->
