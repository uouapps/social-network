...
ob_start();
class chilepro_Widget_Newsletter extends WP_Widget {
	/**
	...
	function __construct() {
		...
		$widget_options = array( 
			...
			'description' => esc_html__( 'Displays a subscription form for your Google/Feedburner account.', 'chilepro' ) 
		);
		/* Set up the widget control options. */
		$control_options = array( 
			'width' => 200, 
			'height' => 350, 
			'id_base' => 'newsletter-chilepro'
		);
		/* Create the widget. */
		// $this->WP_Widget( 'newsletter', __( 'Newsletter', 'unique' ), $widget_options, $control_options );
		parent::__construct('newsletter-chilepro', esc_html__('chilepro Newsletter','chilepro' ), $widget_options, $control_options);
...
<?php global $sb_option_data; ?>

<?php if(($sb_option_data['sb-multi-footer-image']==5)||($sb_option_data['sb-multi-footer-image']==6)){?>
<?php } else {?>
<div class="col-md-3 col-sm-6">
<?php } ?>

          <h5><?php esc_html_e('Newsletter','chilepro'); ?></h5>
<?php if(($sb_option_data['sb-multi-footer-image']==5)||($sb_option_data['sb-multi-footer-image']==6)){?>
<?php } else{?>
<p><?php esc_html_e(' Subscribe to our newsletter to receive our latest news and updates. We do not spam.','chilepro'); ?></p>
<?php } ?>
...
<?php
	...
	function form( $instance ) {
		//Defaults
		$defaults = array(
			'news_title' => esc_html__( 'Newsletter', 'unique' ),
			'input_text' => esc_html__( 'info@example.com', 'unique' ),
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'news_title' )); ?>"><?php esc_html_e( 'Title:', 'chilepro' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'feedburner_id' )); ?>"><?php esc_html_e( 'Google/Feedburner ID:', 'chilepro' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'input_text' )); ?>"><?php esc_html_e( 'Input Text:', 'chilepro' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'submit_text' )); ?>"><?php esc_html_e( 'Submit Text:', 'chilepro' ); ?></label>
...
add_action('widgets_init', 'chilepro_Widget_Newsletter');

function chilepro_Widget_Newsletter(){

    register_widget('chilepro_Widget_Newsletter');
