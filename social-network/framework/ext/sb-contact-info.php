 ...
 * @since 2.8.0
 */
class sb_Contact_Info extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'sb_widget_contact_info', 'description' => esc_html__('sb Contact Info widgets','chilepro'));
		$control_ops = array('width' => 400, 'height' => 350);
		parent::__construct('sb_contact_info', esc_html__('chilepro Contact Info','chilepro'), $widget_ops, $control_ops);
	}
	...
	public function widget( $args, $instance ) {
		...
		if(isset($chameleon_option_data['chameleon-footer-icon']) && !empty($chameleon_option_data['chameleon-footer-icon'])){
			$logo = $chameleon_option_data['chameleon-footer-icon']['url'];	
		}
...
?>
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','chilepro'); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id('bg_image')); ?>"><?php esc_html_e('Background Image:','chilepro'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('address')); ?>"><?php esc_html_e('Address:','chilepro'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('phone_no')); ?>"><?php esc_html_e('Phone No:','chilepro'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php esc_html_e('Email :','chilepro'); ?></label>
		...
		<p><input id="<?php echo esc_attr($this->get_field_id('filter')); ?>" name="<?php echo esc_attr($this->get_field_name('filter')); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo esc_attr($this->get_field_id('filter')); ?>"><?php esc_html_e('Automatically add paragraphs','chilepro'); ?></label></p>
<?php
...
add_action('widgets_init', 'sb_contact_info');

function sb_contact_info(){

    register_widget('sb_Contact_Info');
