...
wp_enqueue_style('all-post-style', SB_CSS.'all-post.css', array(), $ver = false, $media = 'all');
?>    
                  ...
                  <div class="portlet-body">
					...
					<div class="table-responsive listing-table">
					 <table class="table table-striped ">
							...
							<tr class="table-head">
								<th><?php  esc_html_e('Title','chilepro');?></th>								
								<th><?php  esc_html_e('Status','chilepro');?></th>
								<th><?php  esc_html_e('Type','chilepro');?></th>
								<th><?php  esc_html_e('Expire','chilepro');?></th>
								<th><?php  esc_html_e('Actions','chilepro');?></th>
							</tr>
							...
							<?php
								...
								global $wpdb;
									...
									$sql="SELECT * FROM $wpdb->posts WHERE post_type IN ('hospital','doctor' )  and post_author='".$current_user->ID."' and post_status IN ('publish','pending','draft' )  ORDER BY `ID` DESC limit ".$row_strat.", ".$row_end;									
									...
									if($total_post>0){
										...
										?>
											<tr>
												...
												<td width="20%" >
													<?php										
														$edit_post= $profile_url.'?&profile='.$row->post_type.'-edit&post-id='.$row->ID;										
														?>											
								...
								<?php 
										...
										<tr>
											<td colspan="100%">
											<?php esc_html_e('Currently you have no listings added. Please manage your account from the sidebar on the left.','chilepro'); ?>
											</td>
										</tr>
									<?php
									}	
							...
							<div class="center">
								<?php
								$sql2="SELECT * FROM $wpdb->posts WHERE post_type IN ('hospital','doctor' )  and post_author='".$current_user->ID."' and post_status IN ('publish','pending','draft' ) ";
