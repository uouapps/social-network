			...
			if($package_cost >0){
					...
					if($iv_directories_package_recurring!='on'){  // Not recurring
						...
						$package_amount=get_post_meta($package_id, 'iv_directories_package_cost',true);
							$post_count = $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE post_title = '" . $coupon_code . "' and  post_type='chilepro_coupon'");	
							...
							if(sizeof($post_count)>0){
								 ...
								 $start_date=get_post_meta($post_count->ID, 'chilepro_coupon_start_date', true);
								 $end_date=get_post_meta($post_count->ID, 'chilepro_coupon_end_date', true);
								 $coupon_used=get_post_meta($post_count->ID, 'chilepro_coupon_used', true);
								 $coupon_limit=get_post_meta($post_count->ID, 'chilepro_coupon_limit', true);
								 $dis_amount=get_post_meta($post_count->ID, 'chilepro_coupon_amount', true);							 
								 $package_ids =get_post_meta($post_count->ID, 'chilepro_coupon_pac_id', true);
								 ...
								 if($today_time >= $start_time && $today_time<=$expire_time && $coupon_used<=$coupon_limit && $pac_found == '1' ){	
									$coupon_type= get_post_meta($post_count->ID, 'chilepro_coupon_type', true);
									update_post_meta($post_count->ID, 'chilepro_coupon_used', $coupon_used+1);
